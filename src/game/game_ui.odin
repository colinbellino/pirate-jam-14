...
draw_debug_windows :: proc(app: ^engine.App, game: ^Game_State) {
    ...
    if app.config.HOT_RELOAD_CODE && time.diff(app.debug.last_reload, time.now()) < time.Millisecond * 1000 {
        if engine.ui_window("Code reloaded", { game.window_size.x - 190, game.window_size.y - 80, 170, 60 }, { .NO_CLOSE, .NO_RESIZE }) {
            engine.ui_layout_row({ -1 }, 0);
            engine.ui_label(fmt.tprintf("Reloaded at: %v", time.time_to_unix(app.debug.last_reload)));
        }
    }

    if game.debug_ui_window_info {
        if engine.ui_window("Debug", { 0, 0, 500, game.window_size.y }, { .NO_CLOSE }) {
            ...
            if .ACTIVE in engine.ui_header("Config", { .EXPANDED }) {
                ...
                engine.ui_label(fmt.tprintf("%v", time.time_to_unix(app.debug.last_reload)));
                engine.ui_label("TRACY_ENABLE");
                engine.ui_label(fmt.tprintf("%v", app.config.TRACY_ENABLE));
                engine.ui_label("HOT_RELOAD_CODE");
                engine.ui_label(fmt.tprintf("%v", app.config.HOT_RELOAD_CODE));
                engine.ui_label("HOT_RELOAD_ASSETS");
                engine.ui_label(fmt.tprintf("%v", app.config.HOT_RELOAD_ASSETS));
                engine.ui_label("ASSETS_PATH");
                engine.ui_label(fmt.tprintf("%v", app.config.ASSETS_PATH));
            }

            if .ACTIVE in engine.ui_header("Game", { .EXPANDED }) {
                engine.ui_layout_row({ 170, -1 }, 0);
                engine.ui_label("window_size");
                engine.ui_label(fmt.tprintf("%v", game.window_size));
                engine.ui_label("FPS");
                engine.ui_label(fmt.tprintf("%v", u32(1 / app.platform.prev_frame_duration)));
                engine.ui_label("Game_Mode");
                engine.ui_label(fmt.tprintf("%v", game.game_mode));
                // engine.ui_label("draw_letterbox");
                // engine.ui_label(fmt.tprintf("%v", game.draw_letterbox));
                // engine.ui_label("mouse_screen_position");
                // engine.ui_label(fmt.tprintf("%v", game.mouse_screen_position));
                // engine.ui_label("mouse_grid_position");
                // engine.ui_label(fmt.tprintf("%v", game.mouse_grid_position));
                // engine.ui_label("current_room_index");
                // engine.ui_label(fmt.tprintf("%v", game.current_room_index));
                // engine.ui_label("party");
                // engine.ui_label(fmt.tprintf("%v", game.party));
            }

            if .ACTIVE in engine.ui_header("Debug", { .EXPANDED }) {
                engine.ui_layout_row({ 170, -1 });
                engine.ui_label("debug_ui_window_info");
                engine.ui_label(fmt.tprintf("%v", game.debug_ui_window_info));
                engine.ui_label("debug_ui_window_entities");
                engine.ui_label(fmt.tprintf("%v", game.debug_ui_window_entities));
                engine.ui_label("debug_ui_no_tiles");
                engine.ui_label(fmt.tprintf("%v", game.debug_ui_no_tiles));
                engine.ui_label("debug_ui_room_only");
                engine.ui_label(fmt.tprintf("%v", game.debug_ui_room_only));
                engine.ui_label("debug_ui_entity");
                engine.ui_label(fmt.tprintf("%v", game.debug_ui_entity));
                engine.ui_label("debug_ui_show_tiles");
                engine.ui_label(fmt.tprintf("%v", game.debug_ui_show_tiles));
                engine.ui_label("debug_show_bounding_boxes");
                engine.ui_label(fmt.tprintf("%v", game.debug_show_bounding_boxes));
                engine.ui_label("debug_entity_under_mouse");
                engine.ui_label(fmt.tprintf("%v", game.debug_entity_under_mouse));
            }
            ...
            if .ACTIVE in engine.ui_header("Platform", { .EXPANDED }) {
                ...
                engine.ui_label(fmt.tprintf("%v", app.platform.mouse_position));
                engine.ui_label("unlock_framerate");
                engine.ui_label(fmt.tprintf("%v", app.platform.unlock_framerate));
                ...
                if .ACTIVE in engine.ui_treenode("Keyboard", { }) {
                    ...
                    for key in keys {
                        engine.ui_label(fmt.tprintf("%v", key));
                        engine.ui_label(fmt.tprintf("%v", app.platform.keys[key].down));
                        engine.ui_label(fmt.tprintf("%v", !app.platform.keys[key].down));
                        engine.ui_label(fmt.tprintf("%v", app.platform.keys[key].pressed));
                        engine.ui_label(fmt.tprintf("%v", app.platform.keys[key].released));
                    }
                }
            }
            ...
            if .ACTIVE in engine.ui_header("Renderer", { .EXPANDED }) {
                ...
                engine.ui_label(fmt.tprintf("%v", app.platform.update_rate));
                engine.ui_label("display_dpi");
                engine.ui_label(fmt.tprintf("%v", app.renderer.display_dpi));
                engine.ui_label("rendering_size");
                engine.ui_label(fmt.tprintf("%v", app.renderer.rendering_size));
                engine.ui_label("rendering_scale");
                engine.ui_label(fmt.tprintf("%v", app.renderer.rendering_scale));
                engine.ui_label("rendering_offset");
                engine.ui_label(fmt.tprintf("%v", app.renderer.rendering_offset));
                ...
                engine.ui_label(fmt.tprintf("%v", len(app.renderer.textures)));
            }
        }
    }
    ...
    if game.debug_ui_window_entities {
        if engine.ui_window("Entities", { game.window_size.x - 360, 0, 360, 640 }, { .NO_CLOSE }) {
            engine.ui_layout_row({ 160, -1 }, 0);

            engine.ui_label("entities");
            engine.ui_label(fmt.tprintf("%v", len(game.entities.entities)));

            engine.ui_layout_row({ 160, -1 }, 0);
            engine.ui_checkbox("Show room only", &game.debug_ui_room_only);

            engine.ui_layout_row({ 160, -1 }, 0);
            engine.ui_checkbox("Hide tiles", &game.debug_ui_no_tiles);
            ...
            for entity in game.entities.entities {
                ...
                engine.ui_label(fmt.tprintf("%v", entity_format(entity, &game.entities)));
        ...
        if game.debug_ui_entity != 0 {
            ...
            if engine.ui_window(fmt.tprintf("Entity %v", entity), { game.window_size.x - 360 - 360, 0, 360, 640 }, { .NO_CLOSE }) {
