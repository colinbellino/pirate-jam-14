...
draw_debug_windows :: proc() {
    if _game.app.renderer.rendering_size == 0 {
        return;
    }

    if _game.app.config.HOT_RELOAD_CODE && time.diff(_game.app.debug.last_reload, time.now()) < time.Millisecond * 1000 {
        if engine.ui_window("Code reloaded", { _game.window_size.x - 190, _game.window_size.y - 80, 170, 60 }, { .NO_CLOSE, .NO_RESIZE }) {
            ...
            engine.ui_label(fmt.tprintf("Reloaded at: %v", time.time_to_unix(_game.app.debug.last_reload)));
        }
    }
    ...
    if _game.debug_ui_window_info {
        if engine.ui_window("Debug", { 0, 0, 500, _game.window_size.y }, { .NO_CLOSE }) {
            ...
            if .ACTIVE in engine.ui_header("Config", { .EXPANDED }) {
                ...
                engine.ui_label(fmt.tprintf("%v", time.time_to_unix(_game.app.debug.last_reload)));
                engine.ui_label("TRACY_ENABLE");
                engine.ui_label(fmt.tprintf("%v", _game.app.config.TRACY_ENABLE));
                engine.ui_label("HOT_RELOAD_CODE");
                engine.ui_label(fmt.tprintf("%v", _game.app.config.HOT_RELOAD_CODE));
                engine.ui_label("HOT_RELOAD_ASSETS");
                engine.ui_label(fmt.tprintf("%v", _game.app.config.HOT_RELOAD_ASSETS));
                engine.ui_label("ASSETS_PATH");
                engine.ui_label(fmt.tprintf("%v", _game.app.config.ASSETS_PATH));
            }
            ...
            if .ACTIVE in engine.ui_header("Game", { .EXPANDED }) {
                ...
                engine.ui_label("FPS");
                engine.ui_label(fmt.tprintf("%v", u32(1 / _game.app.platform.prev_frame_duration)));
            ...
            if .ACTIVE in engine.ui_header("Assets", { }) {
                ...
                for i := 0; i < _game.app.assets.assets_count; i += 1 {
                    asset := &_game.app.assets.assets[i];
            ...
            if .ACTIVE in engine.ui_header("Platform", { .EXPANDED }) {
                ...
                engine.ui_label(fmt.tprintf("%v", _game.app.platform.mouse_position));
                engine.ui_label("unlock_framerate");
                engine.ui_label(fmt.tprintf("%v", _game.app.platform.unlock_framerate));
                ...
                if .ACTIVE in engine.ui_treenode("Controllers", { }) {
                    ...
                    for joystick_id, controller_state in _game.app.platform.controllers {
                ...
                if .ACTIVE in engine.ui_treenode("Keyboard", { }) {
                    ...
                    for key in keys {
                        engine.ui_label(fmt.tprintf("%v", key));
                        engine.ui_label(fmt.tprintf("%v", _game.app.platform.keys[key].down));
                        engine.ui_label(fmt.tprintf("%v", !_game.app.platform.keys[key].down));
                        engine.ui_label(fmt.tprintf("%v", _game.app.platform.keys[key].pressed));
                        engine.ui_label(fmt.tprintf("%v", _game.app.platform.keys[key].released));
                    }
                }
            }
            ...
            if .ACTIVE in engine.ui_header("Renderer", { .EXPANDED }) {
                ...
                engine.ui_label(fmt.tprintf("%v", _game.app.platform.update_rate));
                engine.ui_label("display_dpi");
                engine.ui_label(fmt.tprintf("%v", _game.app.renderer.display_dpi));
                engine.ui_label("rendering_size");
                engine.ui_label(fmt.tprintf("%v", _game.app.renderer.rendering_size));
                engine.ui_label("rendering_scale");
                engine.ui_label(fmt.tprintf("%v", _game.app.renderer.rendering_scale));
                engine.ui_label("rendering_offset");
                engine.ui_label(fmt.tprintf("%v", _game.app.renderer.rendering_offset));
                ...
                for scale in scales {
                    if .SUBMIT in engine.ui_button(fmt.tprintf("x%i", scale)) {
                        ...
                        _game.app.renderer.rendering_scale = scale;
                        update_rendering_offset(_game.app.renderer);
                    }
                }
                engine.ui_layout_row({ 170, -1 }, 0);
                engine.ui_label("textures");
                engine.ui_label(fmt.tprintf("%v", len(_game.app.renderer.textures)));
            }
        }
    }
