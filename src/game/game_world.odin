        ...
        case .Explore: {
...

            {
                ...
                if move_input.x != 0 ||  move_input.y != 0 {
                    entity_move_grid(camera_position, camera_position.grid_position + move_input);
                }
            }
        }
...
make_world :: proc(data: ^ldtk.LDTK, allocator: runtime.Allocator = context.allocator) -> World {
    context.allocator = allocator;

    world_size := Vector2i { 2, 2 };
    ...
    //         }
    //     }
    //     assert(room_index > -1, fmt.tprintf("Can't find level with identifier: %v%v", ROOM_PREFIX, room_id));
        ...
        grid := make([]i32, room_size.x * room_size.y);
        ...
        // Entity instances
        entity_instances: []ldtk.EntityInstance;
        if len(level.layerInstances) - 1 >= LDTK_ENTITY_LAYER {
