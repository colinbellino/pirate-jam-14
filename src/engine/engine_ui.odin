...
ui_init :: proc() -> (ok: bool) {
    ...
    _app.ui = new(UI_State);
    _app.ui.rendering_offset = &_app.renderer.rendering_offset;
    ...
    _app.ui.atlas_texture = atlas_texture;
    ...
    mu.init(&_app.ui.ctx);
    _app.ui.ctx.text_width = mu.default_atlas_text_width;
    _app.ui.ctx.text_height = mu.default_atlas_text_height;
...
ui_process_commands :: proc() {
    ...
    for variant in mu.next_command_iterator(&_app.ui.ctx, &command_backing) {
...
ui_is_hovered :: proc() -> bool {
    return _app.ui.hovered;
}

// begin -> draw -> end -> process_commands -> present
ui_begin :: proc() {
    mu.begin(&_app.ui.ctx);
}
ui_end :: proc() {
    mu.end(&_app.ui.ctx);
    _app.ui.hovered = false;
}

ui_input_mouse_move :: proc(x: i32, y: i32) {
    mu.input_mouse_move(&_app.ui.ctx, x, y);
}

ui_input_scroll :: proc(x: i32, y: i32) {
    mu.input_scroll(&_app.ui.ctx, x, -y);
}

ui_input_text :: proc(text: string) {
    mu.input_text(&_app.ui.ctx, text);
}

ui_input_mouse_down :: proc(x: i32, y: i32, button: Mouse) {
    mu.input_mouse_down(&_app.ui.ctx, x, y, button);
}
ui_input_mouse_up :: proc(x: i32, y: i32, button: Mouse) {
    mu.input_mouse_up(&_app.ui.ctx, x, y, button);
}

ui_input_key_down :: proc(key: Key) {
    mu.input_key_down(&_app.ui.ctx, key);
}
ui_input_key_up :: proc(key: Key) {
    mu.input_key_up(&_app.ui.ctx, key);
}

ui_u8_slider :: proc(val: ^u8, lo, hi: u8) -> (res: Result_Set) {
    mu.push_id(&_app.ui.ctx, uintptr(val));

    @static tmp: mu.Real;
    tmp = mu.Real(val^);
    res = mu.slider(&_app.ui.ctx, &tmp, mu.Real(lo), mu.Real(hi), 0, "%.0f", {.ALIGN_CENTER});
    val^ = u8(tmp);
    mu.pop_id(&_app.ui.ctx);
    return;
}

ui_mouse_over :: proc(rect: Rect, opt: Options) -> bool {
    if .NO_INTERACT in opt {
        return false;
    }
    return mu.mouse_over(&_app.ui.ctx, cast(mu.Rect) rect);
}

ui_begin_window :: proc(title: string, rect: Rect, opt := Options{}) -> bool {
    return mu.begin_window(&_app.ui.ctx, title, cast(mu.Rect) rect, opt);
}
...
ui_scoped_end_window :: proc(title: string, rect: Rect, opt: Options, opened: bool) {
    if opened {
        mu.scoped_end_window(&_app.ui.ctx, title, cast(mu.Rect) rect, opt, opened);
    }
}

ui_button :: proc(label: string, icon: Icon = .NONE) -> Result_Set {
    return mu.button(&_app.ui.ctx, label);
}

ui_label :: proc(text: string) {
    mu.label(&_app.ui.ctx, text);
}

ui_panel_begin :: proc(name: string, opt := Options {}) {
    mu.begin_panel(&_app.ui.ctx, name, opt);
}

ui_panel_end :: proc() {
    mu.end_panel(&_app.ui.ctx);
}

ui_text :: proc(text: string) {
    mu.text(&_app.ui.ctx, text);
}

ui_get_current_container :: proc() -> ^Container {
    return mu.get_current_container(&_app.ui.ctx);
}

ui_textbox :: proc(buf: []u8, textlen: ^int, opt := Options{}) -> Result_Set {
    return mu.textbox(&_app.ui.ctx, buf, textlen, opt);
}

ui_set_focus :: proc(id: Id) {
    mu.set_focus(&_app.ui.ctx, id);
}

ui_checkbox :: proc(label: string, state: ^bool) -> (res: Result_Set) {
    return mu.checkbox(&_app.ui.ctx, label, state);
}

ui_push_id_uintptr :: proc(ptr: uintptr) {
    mu.push_id_uintptr(&_app.ui.ctx, ptr);
}
...
ui_draw_rect :: proc(rect: Rect, color: Color) {
    mu.draw_rect(&_app.ui.ctx, cast(mu.Rect) rect, cast(mu.Color) color);
}

ui_get_layout :: proc() -> ^Layout {
    return mu.get_layout(&_app.ui.ctx);
}

ui_layout_next :: proc() -> Rect {
    return cast(Rect) mu.layout_next(&_app.ui.ctx);
}
...
ui_layout_row :: proc(widths: []i32, height: i32 = 0) {
    mu.layout_row(&_app.ui.ctx, widths, height);
}
ui_layout_column :: proc() -> bool {
    return mu.layout_column(&_app.ui.ctx);
}
ui_layout_width :: proc(width: i32) {
    mu.layout_width(&_app.ui.ctx, width);
}

ui_header :: proc(label: string, opt := Options{}) -> Result_Set {
    return mu.header(&_app.ui.ctx, label, opt);
}
...
ui_scoped_end_treenode :: proc(label: string, opt: Options, result_set: Result_Set) {
    mu.scoped_end_treenode(&_app.ui.ctx, label, opt, result_set);
}
ui_treenode_begin :: proc(label: string, opt := Options{}) -> Result_Set {
    return mu.begin_treenode(&_app.ui.ctx, label, opt);
}
ui_treenode_end :: proc() {
    mu.end_treenode(&_app.ui.ctx);
}
