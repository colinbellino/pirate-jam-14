...
// game_code_load :: proc(path: string) -> (bool) {
...
//     _app.debug.last_reload = time.now();
//     _game_library = game_library;

//     _app.debug.game_counter += 1;
//     log.debugf("%v loaded: %v, %v, %v, %v.", path, _game_library, _game_update_proc, _game_fixed_update_proc, _game_render_proc);
//     return true;
// }

// game_code_reload_init :: proc() {
//     dir := slashpath.dir(_app.config.os_args[0], context.temp_allocator);
//     file_name := fmt.tprintf("game%i.bin", 0);
//     asset_id := asset_add(file_name, .Code);
//     asset_load(asset_id);
// }

// @(private="file")
// _game_update_proc_stub : Update_Proc : proc(delta_time: f64) {
