...
when RENDERER == .SDL {
    ...
    renderer_init :: proc(window: ^Window, allocator: mem.Allocator, vsync: bool = false) -> (ok: bool) {
        ...
        if PROFILER {
            _e.renderer.arena = cast(^mem.Arena)(cast(^ProfiledAllocatorData)allocator.data).backing_allocator.data
        } else {
            _e.renderer.arena = cast(^mem.Arena)allocator.data
        }
        ...
        _e.renderer.renderer = sdl2.CreateRenderer(window, backend_index, { .ACCELERATED, .PRESENTVSYNC })
        if _e.renderer.renderer == nil {
        ...
        assert(_e.ui != nil, "ui not initialized correctly!")
    ...
    renderer_clear :: proc(color: Color) {
        ...
        sdl2.RenderClear(_e.renderer.renderer)
    }

    renderer_present :: proc() {
        sdl2.RenderPresent(_e.renderer.renderer)
    }
    ...
    renderer_draw_texture_by_index :: proc(texture_index: int, source: ^Rect, destination: ^RectF32, flip: RendererFlip = .NONE, color: Color = { 255, 255, 255, 255 }) {
        assert(texture_index < len(_e.renderer.textures), fmt.tprintf("Texture out of bounds: %v", texture_index))
        texture := _e.renderer.textures[texture_index]
    ...
    renderer_draw_texture_by_ptr :: proc(texture: ^Texture, source: ^Rect, destination: ^RectF32, flip: RendererFlip = .NONE, color: Color = { 255, 255, 255, 255 }) {
        _apply_scale(destination, _e.renderer.rendering_scale)
        _apply_offset(destination, _e.renderer.rendering_offset)
        _apply_pixel_density(destination, _e.renderer.pixel_density)
        sdl2.SetTextureAlphaMod(texture, color.a)
        sdl2.SetTextureColorMod(texture, color.r, color.g, color.b)
        sdl2.RenderCopyExF(_e.renderer.renderer, texture, source, destination, 0, nil, flip)
    }
    ...
    renderer_draw_texture_no_offset :: proc(texture: ^Texture, source: ^Rect, destination: ^RectF32, color: Color = { 255, 255, 255, 255 }) {
        ...
        sdl2.SetTextureAlphaMod(texture, color.a)
        sdl2.SetTextureColorMod(texture, color.r, color.g, color.b)
        sdl2.RenderCopy(_e.renderer.renderer, texture, source, &{ i32(destination.x), i32(destination.y), i32(destination.w), i32(destination.h) })
    }

    renderer_set_draw_color :: proc(color: Color) -> i32 {
        return sdl2.SetRenderDrawColor(_e.renderer.renderer, color.r, color.g, color.b, color.a)
    }
    ...
    renderer_draw_fill_rect_f32 :: proc(destination: ^RectF32, color: Color) {
        ...
        _apply_pixel_density(destination, _e.renderer.pixel_density)
        renderer_set_draw_color(color)
        sdl2.SetRenderDrawBlendMode(_e.renderer.renderer, .BLEND)
        sdl2.RenderFillRectF(_e.renderer.renderer, destination)
    }
    ...
    renderer_draw_fill_rect_no_offset :: proc(destination: ^RectF32, color: Color) {
        ...
        sdl2.RenderFillRect(_e.renderer.renderer, &{ i32(destination.x), i32(destination.y), i32(destination.w), i32(destination.h) })
    }
    ...
    renderer_set_clip_rect :: proc(rect: ^Rect) {
        sdl2.RenderSetClipRect(_e.renderer.renderer, rect)
    }

    // renderer_take_screenshot :: proc(window: ^Window) {
    //     width : i32
    //     height : i32
    //     sdl2.GetRendererOutputSize(_e.renderer.renderer, &width, &height)
    ...
    //     sdl2.RenderReadPixels(_e.renderer.renderer, {}, surface.format.format, surface.pixels, surface.pitch)
    ...
    renderer_read_pixels :: proc(rect: ^Rect, format: sdl2.PixelFormatEnum, pixels: rawptr, pitch: i32) {
        result := sdl2.RenderReadPixels(_e.renderer.renderer, rect, u32(format), pixels, pitch)
        if result < 0 {
    ...
    renderer_create_texture_from_surface :: proc (surface: ^Surface) -> (texture: ^Texture, texture_index: int = -1, ok: bool) {
        texture = sdl2.CreateTextureFromSurface(_e.renderer.renderer, surface)
        if texture == nil {
            log.errorf("Couldn't convert image to texture.")
            return
        }

        append(&_e.renderer.textures, texture)
        texture_index = len(_e.renderer.textures) - 1
        ok = true
    ...
    renderer_create_texture :: proc(pixel_format: u32, texture_access: TextureAccess, width: i32, height: i32) -> (texture: ^Texture, texture_index: int = -1, ok: bool) {
        context.allocator = _e.renderer.allocator

        texture = sdl2.CreateTexture(_e.renderer.renderer, pixel_format, texture_access, width, height)
        if texture == nil {
    ...
    renderer_get_window_pixel_density :: proc(window: ^Window) -> f32 {
        ...
        output_width : i32 = 0
        output_height : i32 = 0
        sdl2.GetRendererOutputSize(_e.renderer.renderer, &output_width, &output_height)
        return f32(output_width / window_size.x)
    }

    renderer_draw_line :: proc(pos1: ^Vector2i32, pos2: ^Vector2i32) -> i32 {
        _apply_scale(pos1, _e.renderer.rendering_scale)
        _apply_offset(pos1, _e.renderer.rendering_offset)
        _apply_pixel_density(pos1, _e.renderer.pixel_density)
        _apply_scale(pos2, _e.renderer.rendering_scale)
        _apply_offset(pos2, _e.renderer.rendering_offset)
        _apply_pixel_density(pos2, _e.renderer.pixel_density)
        return sdl2.RenderDrawLine(_e.renderer.renderer, pos1.x, pos1.y, pos2.x, pos2.y)
    }
    ...
    renderer_set_render_target :: proc(texture: ^Texture) -> i32 {
        return sdl2.SetRenderTarget(_e.renderer.renderer, texture)
    }
