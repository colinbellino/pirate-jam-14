...
file_watch_add :: proc(asset_id: Asset_Id, callback_proc: File_Watch_Callback_Proc) {
    if _engine.debug.file_watches_count == len(_engine.debug.file_watches) {
        log.error("Max file watch reached.")
        return
    }

    file_watch := File_Watch { asset_id, callback_proc }
    _engine.debug.file_watches[_engine.debug.file_watches_count] = file_watch
    _engine.debug.file_watches_count += 1
}

file_watch_remove :: proc(asset_id: Asset_Id) {
    for i := 0; i < _engine.debug.file_watches_count; i += 1 {
        file_watch := &_engine.debug.file_watches[i]
...
file_watch_update :: proc() {
    for i in 0 ..< _engine.debug.file_watches_count {
        file_watch := &_engine.debug.file_watches[i]
