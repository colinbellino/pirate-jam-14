...
file_watch_add :: proc(asset_id: Asset_Id, callback_proc: File_Watch_Callback_Proc) {
    if _core.file_watches_count == len(_core.file_watches) {
        log.error("Max file watch reached.")
        return
    }

    file_watch := File_Watch { asset_id, callback_proc }
    _core.file_watches[_core.file_watches_count] = file_watch
    _core.file_watches_count += 1
}

file_watch_remove :: proc(asset_id: Asset_Id) {
    for i := 0; i < _core.file_watches_count; i += 1 {
        file_watch := &_core.file_watches[i]
...
file_watch_update :: proc() {
    when HOT_RELOAD_ASSETS {
        ...
        for i in 0 ..< _core.file_watches_count {
            file_watch := &_core.file_watches[i]
