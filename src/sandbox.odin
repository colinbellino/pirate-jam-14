...
main :: proc() {
    ...
    log.debugf("arena:  %v", arena);
    log.debug("write --------------------------------------------------------------------------");
    os.write_entire_file("buffer.bin", buffer, false);
    log.debug("zero  --------------------------------------------------------------------------");
    ...
    log.debug("read  --------------------------------------------------------------------------");
    data, success := os.read_entire_file("buffer.bin");
    mem.copy(&arena.data[0], &data[0], len(buffer));
    log.debugf("buffer: %v", buffer);
