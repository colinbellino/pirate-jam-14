...
odin build ../src/game -build-mode:dll -out:game0.bin "$extra" --max-error-count=1 -disable-assert -no-bounds-check -o=speed -define=TRACY_ENABLE=false -define=PROFILER=false -define=LOG_ALLOC=false -define=HOT_RELOAD_CODE=false -define=HOT_RELOAD_ASSETS=false && \
echo "  Done."
