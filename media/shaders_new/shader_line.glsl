...
vec2 world_to_window_position(vec4 point) {
    ...
    vec4 clip_space_position = projection_matrix * (view_matrix * point);
    vec3 normalized_device_position = clip_space_position.xyz / clip_space_position.w;
    vec2 window_space_position = ((normalized_device_position.xy + 1.0) / 2.0) * window_size + view_offset;
    return window_space_position;
    // return vec2(/* mvp *  */point);
}
...
void main() {
    ...
    for (int i = 1; i < points_count; i += 1) {
        vec2 p1 = world_to_window_position(points[i-1]);
        vec2 p2 = world_to_window_position(points[i]);

        { // Points
            float radius = MARKER_RADIUS * points_radius;
        ...
        { // Lines
            ...
            float r = (THICCNESS * lines_thickness) /* * sin01(time / 200 + length(p4 - p1) * 0.50) */;
            if (length(p4 - p3) < r
                ...
                // float delta = 0.5;
                // frag_color.a = smoothstep(1 - delta, 1 + delta, r);
            }
        }
    }
}
